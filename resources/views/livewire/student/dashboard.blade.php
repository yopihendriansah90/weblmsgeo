<div class="space-y-6">
    <section class="relative overflow-hidden rounded-[28px] border border-[#7a2b1c]/10 bg-gradient-to-br from-[#b64027] via-[#d35a3b] to-[#7a2b1c] p-6 text-white shadow-[0_18px_50px_rgba(195,72,45,0.18)] sm:p-8">
        ...
        <div class="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div class="grid gap-3 sm:grid-cols-3">
                <div class="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#fff1e7]/80">Progres belajar</p>
                    <p class="mt-1 text-3xl font-semibold">{{ $summary['progress_percentage'] }}%</p>
                </div>
                <div class="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#fff1e7]/80">Nilai terbaru</p>
                    <p class="mt-1 text-3xl font-semibold">{{ $summary['latest_score'] ?? '-' }}</p>
                </div>
                <div class="rounded-2xl border border-white/10 bg-white/10 px-4 py-4 backdrop-blur">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#fff1e7]/80">Kuis aktif</p>
        ...
        <div class="relative mt-6 h-3 rounded-full bg-white/15">
            <div class="h-3 rounded-full bg-[#ffd0bd] shadow-[0_0_24px_rgba(255,208,189,0.5)]" style="width: {{ $summary['progress_percentage'] }}%"></div>
        </div>
    </section>
            ...
            <section class="rounded-[24px] border border-slate-200/80 bg-white p-5 shadow-sm">
                        ...
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <div class="flex flex-wrap gap-3">
                                ...
                                @if($summary['last_module']->publishedQuiz)
                                    <a class="inline-flex items-center justify-center rounded-full border border-slate-300 bg-white px-4 py-2.5 text-sm font-medium text-slate-700 transition hover:border-[#db8b73] hover:text-[#b64027]" href="{{ route('student.quizzes.take', $summary['last_module']->publishedQuiz) }}">
                                        Buka Kuis
                                    </a>
                                @endif
                            </div>
                ...
                @else
                    <div class="mt-5 rounded-[22px] border border-dashed border-slate-300 bg-slate-50 p-5">
                        ...
                        <a class="mt-4 inline-flex rounded-full bg-[#c84a2f] px-4 py-2.5 text-sm font-medium text-white shadow-sm shadow-[#c84a2f]/20" href="{{ route('student.courses') }}">Pilih Materi</a>
                    </div>
                @endif
            </section>
